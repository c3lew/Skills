...
產線慣例:每個環節收尾都在票上留「下一步:`/skill #N`」comment。用 `gh` CLI 掃 open tickets,最新活動那張的最後一則交棒 comment 就是答案 — 找到就推薦它,不走路由表。

**Sanity check**:接力棒只回答「那張票的下一步」。指令指向**別張票**時(例:票 A 上寫「下一步:`/build #B`」),先核對票 A 自己到結案條件沒 — 到了就先推 `/close #A`,接力棒指令列第二棒。一張票只有「還在產線上」或「已結案」兩種狀態,沒有「驗完了但放著」。
