...
有一張改不了(撞硬規則、或 client 填的分級打錯字)的時候,`batch.py` 照樣把**整批**的分級印完,那張左欄標成「慢(改不了)」(撞硬規則 —— 車道算得出來就照印)或「改不了」(打錯字 —— 不猜他想打什麼),加理由,然後以非 0 退出、最後那段貼票的行不印。整批都不要貼 —— 先把那張解決(撞硬規則的回去改票的內容,把動到判斷邏輯或資料寫入的那部分切出去;打錯字的請 client 重講一次「快」或「慢」)再整批重跑一次。清單上還在的那幾張是這一輪的分級,不是可以先貼的一半。
