...
薄層 wrap 原件 `/implement`:tdd 循環、typecheck、測試、commit 全依原件,本檔只補三個 delta — **`/code-review` 的並行位置**、**收尾交棒**、**書面證據**。原件檔案不改。

## 1. 呼叫原件

呼叫 `/implement #N`(已收編,模型可叫)跑完整流程。跑的**過程中** `/code-review` 的位置改照 §2(原件序列的那一步不照跑),跑完接 §3 收尾;寫交付物的當下照 §4。

## 2. code-review 的並行位置(delta)

原件把 `/code-review` 寫成 commit 前的那一步(`Once done, use /code-review` → `Commit your work`)。它跟**跑 regression suite**(原件的 full test
...
2. **產出 comment**:改了什麼(commit links)+ code-review findings 處置 + regression suite 結果(紅的指名道姓,§2 指過來的就寫在這裡)。
