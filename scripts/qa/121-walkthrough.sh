...
echo "---- 3b  打錯字那張:車道也照印,而且是票的內容算的 —— 不是收下他打的字"
# #50 覆蓋了一條驗收項,所以車道是慢。他打的是 'fast',要是被收下來就會變成快
# —— 那才是 #108「不猜」要擋的。這格同時咬「有車道」跟「不是他打的那個」。
{ grep -q '^  慢(改不了)  #50 登入頁 — ' "$QA/out.txt"   && ! grep -qE '^  快.*#50' "$QA/out.txt"; }
ok $? "#50 標的是慢(改不了):車道照票的內容算,他打錯的字沒被收下來"

echo "---- 3b2  同樣打錯字、但沒有覆蓋驗收項的票 -> 車道跟著變快"
classify '{"mode":"classify","tickets":[{"number":50,"coverage":[],"override":"fast"}],"titles":{"50":"登入頁"}}'
grep -q '^  快(改不了)  #50 登入頁 — ' "$QA/out.txt"
ok $? "車道真的跟著票的內容跑,不是寫死一個"
