...
rows_n()  { grep -cE '^  (快|慢\(改不了\)|慢|改不了) +#[0-9]+ .* — .+$' "$QA/out.txt"; }
...
grep -nE '^  (快|慢\(改不了\)|慢|改不了) +#[0-9]+' "$QA/out.txt"
{ grep -q '^  慢(改不了)  #49 c — 動到判斷邏輯或資料寫入,硬規則一律慢' "$QA/out.txt" \
  && grep -q '改不成快' "$QA/out.txt"; }
ok $? "被拒那列標成「慢(改不了)」—— 車道照印、指名 #49、帶著理由"
...
{ [ "$(rows_n)" = 3 ] && grep -q '^分級(3 張,其中 2 張改不了)' "$QA/out.txt" \
  && grep -q '^  慢(改不了)  #49' "$QA/out.txt" && grep -q '^  改不了 *#50' "$QA/out.txt" \
  && grep -q '^  慢 *#47' "$QA/out.txt"; }
ok $? "兩張被拒都各自標出來(硬規則那張連車道一起),沒被拒的 #47 照印"

echo "---- 1g  每一行分級都帶一句非空的理由(不是只有快/慢)"
classify "$P_TAIL"
grep -nE '^  (快|慢\(改不了\)|慢|改不了) +#[0-9]+' "$QA/out.txt"
[ "$(grep -cE '^  (快|慢\(改不了\)|慢|改不了) +#[0-9]+ .* — .+$' "$QA/out.txt")" = 3 ]
ok $? "3 行分級,每行都帶一句理由"
